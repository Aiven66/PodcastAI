...
class CosyVoice2(CosyVoice):
    ...
    def __init__(self, model_dir, load_jit=False, load_trt=False, load_vllm=False, fp16=False, trt_concurrent=1):
        ...
        self.frontend = CosyVoiceFrontEnd(configs['get_tokenizer'],
                                          configs['feat_extractor'],
                                          '{}/campplus.onnx'.format(model_dir),
                                          '{}/speech_tokenizer_v2.batch.onnx'.format(model_dir),
                                          '{}/spk2info.pt'.format(model_dir),
                                          configs['allowed_special'])
        self.sample_rate = configs['sample_rate']
        ...
        self.model.load('{}/llm.pt'.format(model_dir),
                        '{}/flow.pt'.format(model_dir),
                        '{}/hift.pt'.format(model_dir))
        # v1.0.53: 注入 frontend 引用，让 llm_job 能访问 tokenizer
        # 用于构建 blacklist（"以前"等高频复读词的 token IDs）
        self.model._frontend = self.frontend
        # On MPS/CPU, convert model to float32 to avoid BFloat16 issues
        if not torch.cuda.is_available():
            self.model.llm = self.model.llm.float()
            self.model.flow = self.model.flow.float()
            self.model.hift = self.model.hift.float()
        if load_vllm:
